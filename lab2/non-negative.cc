/*
    ...
    Using bellman's to find the distance from start to all nodes reachable from start and then just lookup for querries
    Time-complexity O((V+E)log(V)) (from bellman)
...
void bellman(vector<int>& price_to_reach_nodes, unordered_map<int, vector<pair<int, int>>>& outgoing_edges, int start, int n) {
    std::priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> pq; // Min heap
    //Need to write the funciton
...
int main() {
    ...
    while (cin >> n >> m >> q >> s) {
        ...
        bellman(price_to_reach_nodes, outgoing_edges, s, n);
        int node;
        stringstream output;
        for(int i=0; i<q; i++) {
            cin >> node;
